...
/*function processAndDrawChartSleepStagesAnalysis(sleepStagesData) {
    if (!sleepStagesData || sleepStagesData.dataSet.length === 0) {
        console.error("No sleep stage data available.");
        return;
    }

    const sleepStages = { 0: "Invalid", 1: "N3", 2: "Deep", 3: "Light", 4: "REM", 5: "Awake" };
    const stageColors = { 0: "gray", 1: "darkblue", 2: "blue", 3: "lightblue", 4: "purple", 5: "red" };

    const dataSet = sleepStagesData.dataSet;
    const step = Math.max(1, Math.floor(sleepStagesData.dataSetLength / 10)); // Dynamic step size
    const selectedData = dataSet.filter((_, index) => index % step === 0);

    const labels = selectedData.map(entry => formatTimestamp(entry[0]));
    const dataPoints = selectedData.map(entry => entry[1] ?? 0); // Default to "Invalid" (0) if null
    const borderColor = dataPoints.map(stage => stageColors[stage]);

    if (window.hypnogramChartInstance) window.hypnogramChartInstance.destroy();
    ...
    window.hypnogramChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                tooltip: {
                    callbacks: {
                        label: tooltipItem => sleepStages[tooltipItem.raw] || "Unknown"
                    }
                }
            }
        }
