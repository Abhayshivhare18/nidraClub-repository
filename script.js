...
async function fetchDataAndProcess() {
    try {
        const [mappingYaml, jsonResponse] = await Promise.all([
            ...
            fetch("nidraClub_data.json").then(res => res.json()) // Fetch JSON DATA
        ]);
       ...
       const lowerHeartPercentile = getValue(nidraClubData,
        ...
        populateSessionTable(sleepEfficiency,totalSleepTime,
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    //     const nidraClubJsonData = await fetch("nidraClub_data.json");
    //     const nidraClubData = await nidraClubJsonData.json();
         
        
    //     //Actigram graph
    //      processAndDrawChart(nidraClubData.sleepAggregations.sleepAwakeDataset);

    //     //Heart Rate Trend
    //      drawHeartRateChart(nidraClubData.heartRateAggregations.heartRateDataset, nidraClubData.heartRateAggregations.lowerPercentile,
    //          nidraClubData.heartRateAggregations.upperPercentile)

    //      //Breathing Rate Trend
    //      drawBreathingRateChart(nidraClubData.breathingAggregations.breathingRateDataset,nidraClubData.breathingAggregations.lowerPercentile,
    //             nidraClubData.breathingAggregations.upperPercentile)


    //      // Respiratory events per hour of sleep
    //      drawRespiratoryEventsChart(nidraClubData.sleepAggregations.irregularityIndex);    
    //     const date=nidraClubData.date;


    //     //  Respiratory events
    //    populateRespiratoryTable(nidraClubData.sleepAggregations.irregularityIndex,date);
         
    //      //Session summary
    //     populateSessionTable(nidraClubData.sleepAggregations,date);
    //     // Heart Rate Analysis   
    //     populateSessionTable2(nidraClubData.heartRateAggregations,date);

    //     //Breathing Rate Analysis
    //     populateSessionTable3(nidraClubData.breathingAggregations,date);
...
//===========================================
      ...
      //  renderLineChart("sleepChart", jsonData.actigramData, "Sleep Stages");
        ...
        // populateSessionTable(jsonData.sessionSummary);
        // populateSessionTable2(jsonData.sessionSummary2);
        // populateSessionTable3(jsonData.sessionSummary3);
        
                const studyDate = jsonData.DateOfStudy.studyDate;
        // ✅ Render Heart Rate Trend Graph
    //    renderHeartRateTrend(jsonData.heartRateTrend,studyDate );
   //     breathHeartRateTrend(jsonData.breathRateTrend,studyDate);

  

     // renderRespiratoryGraph(jsonData.respiratoryEvents, jsonData.respiratoryEventsGraph);      
